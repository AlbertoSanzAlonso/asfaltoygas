    ...
    <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-10">
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-5">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="aspect-[3/4] bg-secondary/5 animate-pulse" />
          ))}
        </div>
      ) : products && products.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-5">
          {products.slice(0, 5).map((product) => (
            <div key={product.product_id} className="bg-safety/40 p-3 shadow-sm">
              <ProductCard product={product} />
            </div>
          ))}
        </div>
      ) : (
        <div className="text-center py-12 bg-safety/30 border border-secondary/10">
          <p className="font-sans text-secondary/60 text-sm mb-6">
            Pronto habrá productos en outlet.
          </p>
          <Link
            to="/categoria/cascos"
