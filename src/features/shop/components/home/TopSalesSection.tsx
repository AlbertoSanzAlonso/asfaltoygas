...
export const TopSalesSection: React.FC<TopSalesSectionProps> = ({ products, isLoading }) => (
  <section id="novedades" className="relative overflow-hidden py-12 md:py-16 bg-safety">
    <img
      ...
      aria-hidden="true"
      decoding="async"
      className="pointer-events-none select-none absolute inset-0 w-full h-full object-cover object-center opacity-75"
    />
    <div className="relative z-10 max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-10">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8 md:mb-10">
        ...
        >
          Ver todas las novedades <ChevronRight className="w-4 h-4" />
        </Link>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white border border-secondary/10">
          <p className="font-sans text-secondary/60 text-sm mb-6">
            Próximamente nuevas novedades en catálogo.
          </p>
          <Link
            to="/novedades"
            className="inline-flex items-center gap-2 bg-primary text-white font-display font-bold text-sm tracking-[0.2em] uppercase px-8 py-3.5 hover:bg-primary-dark transition-colors"
